...
static void* get_in_addr(sockaddr* sa) //adress
{
...
static std::string get_ip_addr(const sockaddr_in& aClientData)// IP
...
bool server::start()
{
    ...
    {
        std::cerr << "html file did't opened! path - " << path_html_file.c_str() << std::endl;
        return false;
    }
    ...
    {
        std::cerr << " file did't opened! path - " << path_icon_file.c_str() << std::endl;
    }
...
bool server::_ssl_init()
{
    ...
    {
        ERR_print_errors_fp(stderr); //error flow 
        return false;
    }
...
void server::_client_processing(int aSocket, std::string aIp)
{
    ...
    if (SSL_accept(ssl) == -1)      //SSL_accept(ssl) < 0 ??
    {
        ERR_print_errors_fp(stderr);
        SSL_free(ssl);                  
        close(aSocket);
